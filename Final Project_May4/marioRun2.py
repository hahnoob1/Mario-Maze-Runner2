#marioRun2.py
from pygame import*
#python 3.6 (for home)#delete this comment later
from random import*
from math import*
from sys import*
from os import*
#NOTES: When making animations, make the folder and the pic name thes same. just diff number
#Menu-------------------------------------------
def story(MenuOpen):
    running = True
    #Load image
    story = image.load("attachments/story1.png")
...
def credit():
    running = True
    cred = image.load("attachments/credits.png")
...
def instructions():
    running = True
    inst = image.load("attachments/instructions.png")
...
def menu():
    ...
    #Puts in music
    mixer.music.load("attachments/01 - Super Mario Bros.mp3")
    mixer.music.play()
    while running:
        for evnt in event.get():
            if evnt.type == QUIT:
        ...
        ground = image.load("attachments/back.jpg")
        screen.blit(ground,(0,0))
        title = image.load("attachments/Title.png")
        screen.blit(title,(200,30,100,40))
        start = image.load("attachments/start.png")
        screen.blit(start,(280,y*100+150,200,60))
        ints = image.load("attachments/int.png")
        screen.blit(ints,(280,y*100+250,200,60))
        credit = image.load("attachments/credit.png")
        screen.blit(credit,(265,y*100+330,200,60))
        exit1 = image.load("attachments/exit.png")
        screen.blit(exit1,(600,y*100+100,200,60))
...
while running:
    ...
    #MENU-----------------------------------------------
        ...
        while page != "exit":
            if page == "menu":
                page = menu()
            #If page is instructions then page will turn
            #from menu to instructions
